...
def export_to_excel(conn: sqlite3.Connection, excel_path: str = EXCEL_PATH) -> None:
    ...
    common_df = pd.read_sql_query("SELECT * FROM github_common ORDER BY id", conn)
    ...
    try:
        ...
        logger.info(
            "Exported %d raw rows and %d common-schema rows to %s",
            len(raw_df), len(common_df), excel_path,
        )
