...
def test_export_returns_true_on_success(tmp_path):
    conn = get_connection(":memory:")
    upsert_raw(
        conn,
        [{"coin": "bitcoin", "date": "2026-07-30", "metric": "tx_count", "value": 712078, "is_partial": False}],
    ...
    result = export_to_excel(conn, excel_path=str(out_path))
...
def test_export_writes_common_sheet(tmp_path):
    ...
    assert len(common) == 1
    assert common.iloc[0]["id"] == "wallet_metrics:bitcoin:2026-07-30:tx_count"
    conn.close()
